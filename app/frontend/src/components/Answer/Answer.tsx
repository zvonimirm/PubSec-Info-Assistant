...
export const Answer = ({
    ...
    const getTextRuns = (line: string) => {
        const regex = /\*\*(.*?)\*\*/g; // Matches text surrounded by **
        ...
        while ((matches = regex.exec(line)) !== null) {
          ...
          textRuns.push(
            new TextRun({
              text: matches[1], // Extracted text without the **
              bold: true,
            })
          );
    ...
    const getParagraphs = (lines:string[]) =>{
        ...
        return lines.map((line, index) => {
                ...
                const children = getTextRuns(line);
                console.log("ovo je linija: ", line);
